...
export const API_BASE_URL =
  ...
  (import.meta.env.RENDERER_API_BASE_URL ?? "http://127.0.0.1:3000").replace(
    /\/+$/,
    "",
  );
 ...
 * 401 自动刷新重试一次；刷新也失败则清凭证并广播 sola:unauthorized，
 * 由 app-store 把界面切回登录页（client 不反向依赖 store）。
 */
