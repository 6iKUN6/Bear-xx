...
> 状态：已确认的目标架构，尚未实施。当前工程尚未引入 Temporal SDK、Workflow、Worker、AgentFlow Prisma 模型或 `flow.*` 协议事件；聊天任务仍由 API 进程内 producer 执行。
> 日期：2026-08-16。
...
- Temporal Workflow 输入只含 `streamTaskId`、`flowVersionId`、`flowDigest` 和非敏感运行标识。
...
1. Agent Activity 命中 LangGraph interrupt 后，在同一数据库事务创建 `AgentFlowApproval(PENDING)`、`ConversationTurnTraceItem(APPROVAL, RUNNING)`、审批请求 `StreamTaskEvent`，并将 `StreamTask` 投影为 `WAITING_HUMAN`。`AgentFlowApproval.id` 即稳定的 `approvalId`；工具审批与计划审批使用不同 `traceKey`，但都关联同一 `approvalId`。
...
3. 审批 API 锁定 `AgentFlowApproval(PENDING)`，校验任务属主、审批类别与决定参数；在同一事务写入决定、`*.resolved` 事件、trace 收敛结果和唯一的 `AgentFlowSignalOutbox`。相同 `taskId + approvalId` 的同一决定返回既有结果，不同决定返回冲突，不能覆盖首次决定。
...
V1 新增 Prisma 模型 `AgentFlow`、`AgentFlowVersion`、`AgentFlowApproval`、`AgentFlowSignalOutbox`。`AgentFlowVersion.definition` 使用 `Json`，`digest` 使用 SHA-256 十六进制字符串，`schemaVersion` 使用整数。`Agent` 新增可空 `defaultFlowVersionId`。`StreamTask` 新增可空 `flowVersionId`、`flowDigest`、`temporalWorkflowId`、`temporalRunId`；`currentStep` 继续保存当前 nodeKey。`AgentFlowApproval` 至少包含 `taskId`、`runId`、`traceItemId`、`nodeKey`、`kind`、`status`、安全请求摘要、决定、决定时间与操作者，并约束同一 `approvalId` 只能从 `PENDING` 转换一次。`AgentFlowSignalOutbox` 至少包含 `taskId`、`approvalId`、`signalType`、唯一 `dedupeKey`、派发状态、尝试次数与最后错误；它不保存审批决定正文。

完成迁移后的 AgentFlow 任务中，`StreamTask.executionState` 仅保存运行期小型快照，例如 `{ temporal: { runId }, langGraph: { checkpointThreadId }, retry: { attempt } }`。遗留策略任务继续保留既有策略快照；任何任务都不得把完整 FlowDefinition、完整对话或可恢复的业务事实重复塞入该字段。
...
| `POST` | `/admin/agent-flow-versions/:versionId/validate` | 返回结构与能力闭集校验结果 |
| `POST` | `/admin/agent-flow-versions/:versionId/test` | 用 admin 测试会话运行草稿，不得发布 |
...
发布 API 必须使用可串行化事务，保证同一 Flow 同时只有一个 `publishedVersionId`。每次写草稿、导入、发布、回滚写审计记录，审计记录含操作者、时间、版本、digest 和动作，不含敏感 JSON 字段。
...
flow.node.completed    { nodeKey, nodeType, summary, durationMs }
flow.node.failed       { nodeKey, nodeType, category, retryable }
flow.waiting_human     { approvalId, nodeKey, traceKey, approval, expiresAt }
flow.run.resumed       { runSequence, reason }
```

`approval` 是可判别的安全展示载荷：工具审批包含 `kind='tool'`、工具名、脱敏参数、说明和允许决定；计划审批包含 `kind='plan-review'`、步骤、revision 和允许决定。它必须足以复用现有审批卡片，不能只发送节点名后让端侧猜测审批内容。
